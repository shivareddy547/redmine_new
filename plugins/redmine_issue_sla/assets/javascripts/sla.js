$(document).ready(function() {
    ...
    $('#save_response').click(function(){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#respond_msg').show();
                $('#get_respond').hide();
                $('#respond_msg p').replaceWith("<p>Responded by " + data[1] + ", Responded about less than a minute ago  </p><li><ol>"+ data[2]+ "</ol></li><hr    ><br>")
            }
        });
    ...
    $('#Sselecctall').click(function(event) {  //on click
        if(this.checked) { // check select status
...
function trackerSla() {
    $("select#issue_priority_id").empty();
    $("select#issue_status_id").empty();
    $.ajax({
        url: $('#issue_tracker_id').attr('data-url'),
        type: 'GET',
        dataType: "json",
        ...
        success: function(data){
            $.each(data[0], function(i, j){
                ...
                $(row).appendTo("select#issue_priority_id");
            });
            $.each(data[1], function(i, j){
                row = "<option value=\"" + j[0] + "\">" + j[1] + "</option>";
                $(row).appendTo("select#issue_status_id");
            });
        }
    })
}
